...
def cb_1(one, two, three):
    global df, local_time_l, ot_time_l, coord_l, rtt_data_l, line
    #local_time_l.append(time.time())
    #ot_time_l.append(three.timestamp)
    #coord_l.append(two[0].position)
    #rtt_data_l.append(line)
    df = df.append({'local_time': time.time(), 'ot_timestamp': three.timestamp, 'coord': two[0].position}, ignore_index=True)
    print('line', line, 'time', df.ot_timestamp.tail())

global df, local_time_l, ot_time_l, coord_l, rtt_data_l, line

#if you want lists
local_time_l, ot_time_l, coord_l, rtt_data_l = [],[],[],[]

#if you want DataFrame
col_name=['local_time', 'ot_timestamp', 'coord', 'rtt_data' ]
...
time.sleep(1)
with SeggerRTTClient() as client_1:
    for line in client_1:
    #start rtt_connection and write data
